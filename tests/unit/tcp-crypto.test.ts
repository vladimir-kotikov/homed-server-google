import { AES128CBC, deriveMD5Key, DHKeyExchange, padBuffer, unpadBuffer } from '../../src/tcp/crypto';
...
describe('AES128CBC', () => {
  ...
  describe('constructor', () => {
    ...
    it('should throw on invalid key length', () => {
      ...
      expect(() => new AES128CBC(shortKey, testIV)).toThrow('AES-128 key must be 16 bytes');
    });

    it('should throw on invalid IV length', () => {
      const shortIV = Buffer.from('short');
      expect(() => new AES128CBC(testKey, shortIV)).toThrow('AES IV must be 16 bytes');
    });
  });
  ...
  describe('encrypt/decrypt', () => {
    ...
    it('should handle 48-byte data', () => {
      const aes = new AES128CBC(testKey, testIV);
      const plaintext = Buffer.from('012345678901234567890123456789012345678901234567'); // 48 bytes
...
describe('Integration: padBuffer + AES + unpadBuffer', () => {
  ...
  it('should handle JSON data', () => {
    ...
    const json = JSON.stringify({ action: 'test', topic: 'test/topic', message: 'hello' });
